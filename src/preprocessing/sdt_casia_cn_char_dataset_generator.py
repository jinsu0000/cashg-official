...
def _process_single_char(
    ...
    sentence_id = f"casia_cn_{writer_id}_{sample_idx}"
...
def generate_casia_pickles(
    ...
    print("=" * 80)
    print("CASIA_CHINESE → CASHG 데이터셋 변환")
    print("=" * 80)
    ...
    TRAIN_WRITER_OFFSET = 40000
    TEST_WRITER_OFFSET = 42000
    ...
    if process_train:
        ...
        for writer_idx, writer_id in enumerate(tqdm(train_writers, desc="[TRAIN] 작가 처리")):
            ...
            with open(os.path.join(save_style_root, f"{merged_writer_id_str}.pkl"), "wb") as f:
                pickle.dump(writer_chars, f)


            style_rep_map = {ch: [rep] for ch, rep in rep_map.items()
                            if rep.get('image') is not None}
    ...
    print()
    print("=" * 80)
    print(" CASIA_CHINESE 데이터셋 변환 완료!")
    print("=" * 80)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="CASIA_CHINESE → CASHG 데이터셋 변환")
    parser.add_argument("--casia_root", type=str, required=True, help="CASIA_CHINESE 루트 디렉토리")
